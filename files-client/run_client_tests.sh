#!/bin/sh
# TODO/TBD For now only support running against k8s?
# Use mvn to run tests for the files client
#   against the files service running either locally or in k8s
#   and other services (tenants, sk, auth, tokens) running in DEV, STAGING, or PROD
# The files service must already be running.
# NOTE: For safety, for now do not allow running against PROD
# TODO If requested use docker to start up the files service locally using an image
# TODO   - image used is based on TAPIS_RUN_ENV, for example tapis/files:dev
#
# Use mvn to run the files-client integration tests.
#
...
#
# TODO To start up the service locally the following env variables must be set:
...
USAGE2="Usage: Run client tests against service running locally or in K8S."
...
#   to look for service locally.
if [ "$RUN_SVC" = "k8s" ]; then
  export TAPIS_SVC_URL="$TAPIS_BASE_URL"
fi
...
echo "****** Running client tests for service. Target service = $RUN_SVC, TAPIS_RUN_ENV = $RUN_ENV"

# if requested start up the service locally
# TODO if [ "$RUN_SVC" = "local" ]; then
# TODO  echo "Staring service locally"
# TODO  DOCK_RUN_ID=$(./docker_run_svc.sh "${RUN_ENV}")
# TODO  RET_CODE=$?
# TODO  if [ $RET_CODE -ne 0 ]; then
# TODO    echo "======================================================================"
# TODO    echo "Error starting service locally."
# TODO    echo "Exiting ..."
# TODO    echo "======================================================================"
# TODO    exit $RET_CODE
# TODO  fi
...
# If local then stop local service
# TODO if [ "$RUN_SVC" = "local" ]; then
# TODO  echo "Stopping local service using docker container ID: $DOCK_RUN_ID"
