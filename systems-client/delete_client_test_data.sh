...
if [ -z "$TAPIS_DB_PASSWORD" -o -z "TAPIS_DB_JDBCURL" ]; then
...
PGPASSWORD=${TAPIS_DB_PASSWORD} psql --host=${DB_HOST} --port=${DB_PORT} --username=tapis --dbname=${DB_NAME} -q -P pager -c "DELETE FROM tapis_sys.systems WHERE NAME LIKE 'Csys_'"
